...
from autosolver.core.models import CandidateOption, CanonicalInstance
from autosolver.solver.common import apply_option, build_result, can_take_option


def greedy_solve(instance: CanonicalInstance, options: list[CandidateOption], start_time: float) -> tuple[list[CandidateOption], int]:
...
def greedy_result(instance: CanonicalInstance, options: list[CandidateOption], start_time: float):
    selected, elapsed_ms = greedy_solve(instance, options, start_time)
    return build_result(
        instance=instance,
        solver_name="greedy",
        status="ok",
